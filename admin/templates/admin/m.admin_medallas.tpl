									...
									<script type="text/javascript">
										...
										$(function(){
											$('#med_img').change(function(){
												var cssi = $("#med_img option:selected").attr('rel');
												$('#c_icon').attr({"style" : "background: url("+cssi+") no-repeat left center;", "src": cssi});
												console.log(cssi);
											});
											//
										});
										...
										<fieldset>
                                            ...
                                            <dl>
												...
												<dd>
													<img src="{$tsTema.images}/space.gif" style="background:url({$tsConfig.default}/images/icons/med/{if $tsMed.m_image}{$tsMed.m_image}{else}{$tsIcons.0}{/if}_16.png) no-repeat left center;" width="16" height="16" id="c_icon"/>
													<select name="med_img" id="med_img" style="width:164px">
													{foreach from=$tsIcons key=i item=img}
														<option value="{$img}" rel="{$tsConfig.url}/themes/default/images/icons/med/{$img}_16.png" {if $tsMed.m_image == $img}selected="selected"{/if}>{$img}</option>
													{/foreach}
													</select>
												</dd>
											</dl>
											<dl>
										<dt><label for="rSpecial">Condici&oacute;n especial:</label><span>Cuando </span>
