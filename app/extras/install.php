...
// Aqui habilitamos todos los errores
...
define('TS_AVATARES', TS_ROOT . 'assets' . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . 'avatares' . DIRECTORY_SEPARATOR);

if(file_exists(FILE_ENV) AND file_exists(FILE_LOCK)) header("Location: ../");
...
switch ($page) {
	case 'licencia':
		$head = 'Licencia';
		$_SESSION['LICENSE'] = (isset($_POST['agree']) && $_POST['agree'] === 'on') ? true : false;
		if($_SESSION['LICENSE']) {
			/**
			 * Realizamos acciones
			*/
	...
	case 'requisitos':
		...
		$verify = [
			"cache" => str_replace(TS_ROOT, DIRECTORY_SEPARATOR, TS_CACHE),
			"avatar" => str_replace(TS_ROOT, DIRECTORY_SEPARATOR, TS_AVATAR),
			"uploads" => str_replace(TS_ROOT, DIRECTORY_SEPARATOR, TS_UPLOADS),
			"portadas" => str_replace(TS_ROOT, DIRECTORY_SEPARATOR, TS_PORTADAS)
		];
		foreach ($verify as $key => $val) {
			$rutaVal = ".." . DIRECTORY_SEPARATOR . ".." . $val;
	...
	case 'instalacion':
		...
		if(file_exists(TS_ROOT . 'faster.php')) require_once TS_ROOT . 'faster.php';
			...
			try {
		      ...
		      try {
				...
				if (in_array(1, $exe)) {
					...
					$seo = str_replace([' ', ' '], '-', strtolower($titulo));
					# ACTUALIZAMOS LA CATEGORÍA N°33
					$mysqli->query("UPDATE `{$db['prefix']}posts_categorias` SET c_nombre = '$titulo', c_seo = '$seo' WHERE cid = 33 LIMIT 1");
					// Descripción desde faster.php o por defecto
					$description = isset($faster['site']['description']) ? $faster['site']['description'] : 'Únete a nuestra comunidad para compartir experiencias y conocer gente nueva. ¡Conéctate hoy mismo!';
					// Keywords desde faster.php o por defecto
					$keywords = isset($faster['site']['keywords']) ? $faster['site']['keywords'] : 'comunidad, conocer, red, ampliar, interaccion, compartir, amigos, conectar, relaciones, intereses, encuentros, virtual';
					$mysqli->query("UPDATE `{$db['prefix']}seo` SET 
						seo_titulo = '$titulo - {$site['slogan']}', 
						seo_descripcion = '$description', 
						seo_portada = '/portadas/logo-512.webp', 
						seo_keywords = '$keywords' WHERE seo_id = 1");
					...
					if($next_step) {
						...
						# INSERTAMOS USUARIO
							...
							if(!is_dir($folder)) mkdir($folder, 0777, true);
					     	$return_avatar = $folder . DIRECTORY_SEPARATOR . "$uniqid.webp";
			    
					  		# AVATAR ALEATORIO Y CONVIRTIENDO A WEBP
					  		$origen = TS_AVATARES . 'none' . DIRECTORY_SEPARATOR;
							...
							$mysqli->query("INSERT INTO {$db['prefix']}perfil (user_id, user_sexo) VALUES ($uid, 'none')");
							$mysqli->query("INSERT INTO {$db['prefix']}perfil_avatar (`uavatar_id`, `uavatar_use`) VALUES ($uid, '$uniqid')");
...
<body>
	...
	<main>
		...
		<section>
			<form method="POST"<?= ($page === 'finalizar' ? " action=\"$url\"" : '') ?>>
				...
				<?php elseif($page === 'instalacion'): ?>
					...
					<div id="database" class="tab">
						<div class="form-group">
							...
							<input type="text" id="sql_host" name="sql_host" value="<?= isset($faster) ? $faster['db']['host'] : $_POST['sql_host'] ?? '' ?>" placeholder="localhost" required>
						</div>
						<div class="form-group">
							<label for="sql_user">Usuario</label>
							<input type="text" id="sql_user" name="sql_user" value="<?= isset($faster) ? $faster['db']['name'] : $_POST['sql_user'] ?? '' ?>" placeholder="root" required>
						</div>
						<div class="form-group">
							<label for="sql_pass">Contraseña</label>
							<input type="password" id="sql_pass" name="sql_pass" value="<?= isset($faster) ? $faster['db']['pass'] : $_POST['sql_pass'] ?? '' ?>" placeholder="password">
						</div>
						<div class="form-group">
							<label for="sql_name">Base de datos</label>
							<input type="text" id="sql_name" name="sql_name" value="<?= isset($faster) ? $faster['db']['base'] : $_POST['sql_name'] ?? '' ?>" placeholder="name_database" required>
						</div>
						<div class="form-group last-child">
							<label for="sql_prefix">Prefix</label>
							<input type="text" id="sql_prefix" name="sql_prefix" value="<?= isset($faster) ? $faster['db']['prefix'] : $_POST['sql_prefix'] ?? 'zcode_' ?>" placeholder="zcode_">
						</div>
					</div>
					...
					<div id="datasite" class="tab">
						<div class="form-group">
							...
							<input type="text" id="site_title" name="site_title" value="<?= isset($faster) ? $faster['site']['titulo'] : $_POST['site_title'] ?? '' ?>" placeholder="<?= SCRIPT_NAME ?>" required>
						</div>
						<div class="form-group">
							<label for="">Slogan</label>
							<input type="text" id="site_desc" name="site_desc" value="<?= isset($faster) ? $faster['site']['slogan'] : $_POST['site_desc'] ?? '' ?>" placeholder="Actualizando tu mundo" required>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<input type="text" id="site_email" name="site_email" value="<?= isset($faster) ? $faster['site']['email'] : $_POST['site_email'] ?? '' ?>" placeholder="noreply@domain.com">
						</div>
						<div class="form-group">
							<label for="site_pkey">Clave pública</label>
							<input type="text" id="site_pkey" name="site_pkey" value="<?= isset($faster) ? $faster['site']['recaptcha']['public_key'] : $_POST['site_pkey'] ?? '' ?>" placeholder="6LfFFiMdAAAAAAQjDafWXZ0FeyesKYjVm4DSUoao">
						</div>
						<div class="form-group last-child">
							<label for="site_skey">Clave secreta</label>
							<input type="text" id="site_skey" name="site_skey" value="<?= isset($faster) ? $faster['site']['recaptcha']['secret_key'] : $_POST['site_skey'] ?? '' ?>" placeholder="6LfFFiMdAAAAAFIP4oNFLQx5Fo1FyorTzNps8ChE">
						</div>
					...
					<div id="dataadmin" class="tab" tab-active="false">
						<div class="form-group">
							...
							<input type="text" id="admin_username" name="admin_username" maxlength="20" value="<?= isset($faster) ? $faster['admin']['username'] : $_POST['admin_username']?? '' ?>" placeholder="JhonDoe">
						</div>
						<div class="form-group">
							<label for="admin_pass">Contraseña</label>
							<div class="group">
								<input type="password" id="admin_pass" name="admin_pass" value="<?= isset($faster) ? $faster['admin']['password'] : $_POST['admin_pass'] ?? '' ?>" placeholder="password">
						...
						<div class="form-group">
							...
							<div class="group">
								<input type="password" id="admin_confirm_pass" name="admin_confirm_pass" value="<?= isset($faster) ? $faster['admin']['password'] : $_POST['admin_confirm_pass'] ?? '' ?>" placeholder="password">
								<span data-target="#admin_confirm_pass">Mostrar</span>
							</div>
						</div>
						<div class="form-group last-child">
							<label for="admin_email">Email</label>
							<input type="text" id="admin_email" name="admin_email" value="<?= isset($faster) ? $faster['admin']['email'] : $_POST['admin_email'] ?? '' ?>" placeholder="jhondoe@domain.com">
						</div>
					</div>
