...
**/
define('ERROR_LOG_TYPE', false);
require_once realpath(dirname(dirname(__DIR__)) . '/config/app_paths.php');
require_once realpath(dirname(dirname(__DIR__)) . '/config/config_paths.php');

ini_set('error_log', LOG_ERROR_INSTALL);
...
$local = ($_SERVER['QUERY_STRING'] === 'step=') ? dirname($_SERVER["REQUEST_URI"]) : dirname(dirname($_SERVER["REQUEST_URI"]));
...
$script['slogan'] = 'Script a otro nivel!';
$script['version_code'] = SCRIPT_NAME . ' ' . SCRIPT_VERSION;
$script['version_codelow'] = str_replace('.', '_', strtolower($script['version_code']));
$script['forum'] = '<a href="https://phpost.es" title="PHPost" rel="external" target="_blank">PHPost.es</a>';

/**
 * Funciones
*/
function savedata(array $replace = [], array $search = []) {
	$config = file_get_contents(DATABASE_CONNECT);
	$config = str_replace($search, $replace, $config);
	file_put_contents(DATABASE_CONNECT, $config);
}
...
if(!file_exists( DATABASE_CONNECT )) {
	...
	copy(CONFIG_EXAMPLE, DATABASE_CONNECT);
}
// Guardar el contenido actualizado de nuevo en el archivo
file_put_contents(DATABASE_CONNECT, str_replace('__version__', "v" . SCRIPT_VERSION, file_get_contents(DATABASE_CONNECT)));

# 2 - Damos los permisos necesarios
if(file_exists( DATABASE_CONNECT )) chmod(DATABASE_CONNECT, 0666);
...
if( $step === 0 ) {
	...
	$verify = [
		"config" => str_replace(TS_ROOT, TS_PATH, DATABASE_CONNECT),
	...
	if (isset($_POST['save'])) {
      ...
      try {
		...
		$db['installed'] = md5(SCRIPT_NAME . ' Instalado el ' . date('d.m.Y'));

		$db['prefix'] = strtolower($_SESSION['prefix'] ?? SCRIPT_NAME);
		$db['prefix'] .= (substr($db['prefix'], -1) !== '_') ? '_' : '';

		savedata($db, ['dbhost', 'dbuser', 'dbpass', 'dbname', 'dbprefix', 'dbport', 'dbinstalled']);
		...
		$mysqli->close();
		if (!in_array(0, $exe)) header("Location: ./" . ($step + 1));
		else {
	...
	if (isset($_POST['save'])) {
      ...
      foreach($_POST['web'] as $key => $val) $web[$key] = htmlspecialchars($val);
		...
		foreach($tamanos as $tamano) {
			...
			$html = "<a href=\"$github\" target=\"_blank\" style=\"display:block;\"><img loading=\"lazy\" alt=\"Publicidad de $tamano\" title=\"$alt\" width=\"{$size[0]}\" height=\"{$size[1]}\" src=\"$urlBase/assets/images/ad$tamano.webp\"></a>";
	...
	if (isset($_POST['save'])) {
      ...
      foreach($_POST['user'] as $key => $val) $user[$key] = htmlspecialchars($val);
		...
		if($continue) {
            ...
            $mysqli->close();
            header("Location: ./" . ($step + 1) . "?uid=$uid");
			}
	...
	$tsTitle = "Bienvenido a " . SCRIPT_VERSION;
	$tsSubhead = "Finalizar...";
