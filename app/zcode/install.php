...
<link rel="stylesheet" href="<?= $urlBase ?>/assets/css/install.css<?= uniqid('?') ?>">
<link href="<?= $urlBase ?>/assets/images/favicon/logo-32.webp<?= uniqid('?') ?>" rel="icon" type="image/webp">
<title>Instalación de <?= SCRIPT_NAME ?>!</title>
</head>
<body>
	...
	<main>
		<header>
			<div class="container">
				<h1><?= ($step === 5 ? $tsTitle : "Instalando ".SCRIPT_NAME . ' ' . SCRIPT_VERSION."!") ?></h1>
				<h4><?= $tsSubhead ?></h4>
			</div>
		</header>
		<section class="container">
			...
			<form action="<?= $tsAction ?>" method="post" id="form">
				<?php if($step === 0): ?>
					...
					<h3>Licencia</h3>
					<p>Para utilizar <?= SCRIPT_NAME . ' ' . SCRIPT_VERSION ?> debes estar de acuerdo con nuestra licencia de uso.</p>
				...
				<?php elseif ($step === 1): ?>
					...
					<div class="box box-ok" style="font-size:13px;">Se puede continuar con la instalación teniendo la versión de <strong>PHP 7.2+</strong>, pero se recomienda el uso de la versión <strong>PHP 8.2+</strong>, para evitar problemas futuros.</div>
					...
					<div class="medium end">
						<div style="display: grid;grid-template-columns: repeat(2, 1fr);column-gap:1.75rem">
							...
							<div class="form-group">
								...
								<input type="text" name="db[port]" id="puerto" placeholder="3306" value="<?= $db['port'] ?? '' ?>">
					...
					<div class="form-grid">
						<div class="medium">
							...
							<div class="form-group">
								...
								<input type="text" name="web[name]" id="nombre" placeholder="<?= SCRIPT_NAME ?>" value="<?= $web['name'] ?? '' ?>" required>
				...
				<?php elseif ($step === 5): ?>
					...
					<div class="medium end">
						<p>Gracias por instalar <strong><?= SCRIPT_NAME . ' ' . SCRIPT_VERSION ?></strong>. Tu nueva comunidad <strong>Link Sharing System</strong> está lista para ser utilizada. Inicia sesión con tus credenciales para comenzar a disfrutar de todas las funcionalidades. Te invitamos a <a href="https://phpost.es" rel="external" target="_blank">visitarnos</a> regularmente para mantenerte al tanto de futuras actualizaciones. Si encuentras algún error, por favor repórtalo; tu colaboración es fundamental para mejorar el sistema para todos.</p>
		...
		<footer>
			<p><?= SCRIPT_NAME . ' &copy; ' . date('Y') ?> - <strong><?= SCRIPT_AUTHOR ?></strong> - <?= $script['forum'] ?></p>
