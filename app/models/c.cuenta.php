	...
	*/
	public function loadHeadInfo(int $user_id = 0) {
      ...
      // Redes Sociales
		...
		// VISITAS
		include TS_MODELS . 'c.visitas.php';
		$Visitas = new tsVisitas;
   ...
   */
   ...
   # Para activar el doble factor
	public function activeTwoFactor() {
		...
		$authenticator = new \Sonata\GoogleAuthenticator\GoogleAuthenticator();
	  	...
	  	if(db_exec([__FILE__, __LINE__], "query", "UPDATE @miembros SET user_secret_2fa = '{$_POST['secret']}' WHERE user_id = {$this->user->uid}")) {
   ...
   public function saveCuenta() {
      ...
      // ANTIGUOS DATOS
       ...
       // ES EL MISMO CORREO?
		...
		$updates = $this->core->getIUP($perfilData, 'user_');
		if(db_exec([__FILE__, __LINE__], "query", "UPDATE @perfil SET {$updates} WHERE user_id = {$this->user->uid}") OR show_error('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'Base de datos')) $message = 'Los cambios fueron aplicados.';
		return $message;
   }
   ...
   public function savePerfil() {
      ...
      if(!empty($perfilData['sitio']) && !filter_var($perfilData['sitio'], FILTER_VALIDATE_URL)) 
		...
		if(db_exec([__FILE__, __LINE__], "query", "UPDATE @perfil SET {$updates} WHERE user_id = {$this->user->uid}") || show_error('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'Base de datos')) $message = 'Los cambios fueron aplicados.';
