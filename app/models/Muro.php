...
use app\interfaces\MuroInterface;
use app\models\Cuenta;
use app\utils\OpenGraph;

class Muro implements MuroInterface {
	...
	public function getPrivacity(int $user_id = 0, string $username = null, $follow = NULL, $yfollow = NULL){
		...
		switch($data['p_configs']['m']){
			...
			case 5:
				...
				$priv['m']['m'] = "Solo usuarios <a href=\"{$tsCore->settings['url']}/registro/\" rel=\"internal\" class=\"fw-semibold text-decoration-none\">registrados</a> pueden ver el muro de $username";
			break;
		}
		// FIRMAR MURO
		switch($data['p_configs']['mf']){
			...
			case 5:
				...
				$priv['mf']['m'] = "Solo usuarios <a href=\"{$tsCore->settings['url']}/registro/\" rel=\"internal\" class=\"fw-semibold text-decoration-none\">registrados</a> pueden firmar el muro de $username";
			break;
		}
		//
		return $priv;
   ...
   */
   private function validateType($type) {
      if (empty($type)) throw new Exception('0: El campo <strong>type</strong> es obligatorio.');
      return $type;
   }
   ...
   */
   private function sanitizeUrl($url) {
      global $tsCore;
      $url = $tsCore->setSecure($url);
      if (empty($url)) throw new Exception('0: El campo <strong>url</strong> es obligatorio.');
      return $url;
   }
   ...
   */
	private function validatePhoto($url, $return) {
		if (strlen($url) > self::MAX_IMAGE_URL_LENGTH) throw new Exception('0: La url de la imagen es demasiado larga.');
   ...
   */
	private function validateLink($url, $OpenGraph, $return) {
		global $tsCore;
		if (strlen($url) > self::MAX_URL_LENGTH) throw new Exception('0: La url es demasiado larga.');
		...
		$title = $tsCore->setSecure($data['title']);
   ...
   */
	private function validateVideo($url, $OpenGraph, $return) {
		...
		$title = $tsCore->setSecure($data['title']);
		$description = $tsCore->setSecure($data['description']);
		if ($return) {
			return [
				'ID' => $videoId,
				'title' => $title,
   ...
   */
	...
	*/
	public function streamPost(){
		...
		$tsCuenta = new Cuenta();
		$priv = $this->getPrivacity($pid, $exists, $tsCuenta->iyfollow($pid, 'iFollow'), $tsCuenta->iyfollow($pid, 'yFollow'));
		...
		switch($type){
			...
			case 'foto':
				...
				// ANTI FLOOD
				$tsCore->antiFlood();
				$foto = $tsCore->setSecure($foto, true);
				// INSERTAMOS
			...
			case 'enlace':
				...
				// ANTI FLOOD
				$tsCore->antiFlood();
				$title = $tsCore->setSecure($enlace['title'], true);
				$url = $tsCore->setSecure($enlace['url'], true);
				$description = $tsCore->setSecure($tsCore->parseBadWords($enlace['description']));
				// INSERTAR
			...
			case 'video':
				...
				// ANTI FLOOD
				$tsCore->antiFlood();
				$ID = $tsCore->setSecure($video['ID'], true);
				$title = $tsCore->setSecure($video['title'], true);
				$image = $tsCore->setSecure($video['image'], true);
				$description = $tsCore->setSecure($video['description'], true);
				// INSERTAR
		...
		$return = [
			...
			'p_body' => $tsCore->parseBadWords($tsCore->setMenciones($data), true), 
			'p_date' => $date, 
			'p_likes' => 0, 
			'likes' => ['link' => 'Me gusta'],
			...$append_array
	...
	*/
	function streamRepost(){
		...
		$date = time();
		$myIP = $tsCore->executeIP();
		if(!db_exec([__FILE__, __LINE__], 'query', "INSERT INTO @muro_comentarios (`pub_id`, `c_user`, `c_date`, `c_body`, `c_ip`) VALUES ($pid, {$tsUser->uid}, '$date', '$data', '$myIP')")) return '0: '.ShowError('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'db');
	...
	*/
		...
		global $tsUser, $tsCore, $tsZCode;
		  ...
		  switch($type){
				case 'likes':
					 ...
					 // VEMOS SI ME GUSTA
					 if($tsUser->is_member){
						  $query = db_exec([__FILE__, __LINE__], 'query', 'SELECT `like_id` FROM @muro_likes WHERE `user_id` = \''.$tsUser->uid.'\' AND `obj_id` = \''.(int)$pub_id.'\' AND obj_type = \'1\'');
					 ...
					 if($likes == 1){
						  ...
						  if($i_like){
								$data['link'] = 'Ya no me gusta';
								//
								$query = db_exec([__FILE__, __LINE__], 'query', 'SELECT u.user_name FROM @muro_likes AS l LEFT JOIN @miembros AS u ON l.user_id = u.user_id  WHERE l.user_id != \''.$tsUser->uid.'\' AND l.obj_id = \''.(int)$pub_id.'\' AND l.obj_type = 1');
								$u_like = db_exec('fetch_assoc', $query);
				...
				case 'comments':
					 ...
					 while($row = db_exec('fetch_array', $query)){
						  ...
						  if($row['c_likes'] > 0){
								//
								$cuery = db_exec([__FILE__, __LINE__], 'query', 'SELECT `like_id` FROM @muro_likes WHERE `user_id` = \''.$tsUser->uid.'\' AND `obj_id` = \''.$row['cid'].'\' AND `obj_type` = \'2\'');
	...
	public function likePost(){
		...
		// SI AUN NO ME GUSTA
		if(empty($i_like)) {
			if(db_exec([__FILE__, __LINE__], 'query', "INSERT INTO @muro_likes (user_id, obj_id, obj_type) VALUES ({$tsUser->uid}, $id, $type)")) {
				// SUMAR LIKE
				if($type == 1) {
