...
use app\interfaces\MensajesInterface;

class Mensajes implements MensajesInterface {
	 ...
	 */
	 function newMensaje(){
		...
		if(db_exec('num_rows', db_exec([__FILE__, __LINE__], 'query', 'SELECT mp_id FROM @mensajes WHERE (mp_date > \''.(time()-$antiflood).'\' && mp_from = \''.$tsUser->uid.'\') OR (mp_date > \''.(time()-$antiflood*3600).'\' && mp_from = \''.$tsUser->uid.'\' && mp_preview = \''.$mensajito.'\' && `mp_subject` = \''.$tsCore->setSecure($_POST['asunto']).'\') ORDER BY mp_id DESC LIMIT 1'))) die('Espere '.$antiflood.' segundos para continuar'); 
