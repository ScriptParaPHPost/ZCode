...
use app\utils\{Images,Paginator,Zcode};
use app\services\{ContentAccessService,PostCacheService};
   ...
   */
	...
	*/
	public function setNP() {
		...
		$query = db_exec([__FILE__, __LINE__], 'query', "SELECT p.post_id, p.post_user, p.post_category, p.post_title, u.user_name, c.c_nombre, c.c_seo FROM @posts AS p LEFT JOIN @miembros AS u ON p.post_user = u.user_id LEFT JOIN @posts_categorias AS c ON c.cid = p.post_category WHERE p.post_status = 0 AND $isAdmod ORDER BY $order LIMIT 1") or exit(ShowError('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'db'));
	...
	private function getPostStats(&$postData, int $post_id = 0) {
		...
		//ESTADÍSTICAS
			...
			// ACTUALIZAMOS
			$post = $this->buildSqlUpdateFields([
	...
	*/
	public function getPost() {
		...
		$this->access->general($postData, $postData['post_id'], $postData['post_title']);
	...
	private function fetchPostData($post_id) {
		return db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', "SELECT c.* ,m.*, u.user_id FROM @posts AS c LEFT JOIN @miembros AS u ON c.post_user = u.user_id LEFT JOIN @perfil AS m ON c.post_user = m.user_id  WHERE `post_id` = $post_id AND {$this->access->isAdmod()} LIMIT 1"));
	}
	...
	*/
	public function getAutor(int $user_id = 0) {
		...
		// STATUS
		$data['status'] = $this->core->statusUser($user_id);
		// PAIS
	...
	*/
	public function getPunteador(bool $puntuador = false) {
   	$allow = $this->core->settings['c_allow_points'];
    	$data['rango'] = ($allow > 0) ? $allow : ($allow == '-1' ? $this->user->info['user_puntosxdar'] : ($allow == '-2' ? 999 : $this->user->permisos['gopfp'] ?? 0));
	...
	*/
	...
	public function deleteAdminPost(){
		...
		if(!removeDataById([__FILE__, __LINE__], '@posts', "post_id = $pid")) return '0: Ha ocurrido un error eliminando el post.';
		if(!removeDataById([__FILE__, __LINE__], '@posts_comentarios', "c_post_id = $pid")) return '0: Ha ocurrido un error eliminando comentarios del post.';
