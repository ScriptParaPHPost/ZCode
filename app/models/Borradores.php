...
use app\interfaces\BorradoresInterface;
use app\traits\System;

class Borradores implements BorradoresInterface {
		...
		if(!empty($draftData['title'])) {
			if(!empty($draftData['category']) && $draftData['category'] > 0) {
				if($save) {
					...
					else return '0: '.ShowError('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'db');
		   	} else {
					// INSERT
