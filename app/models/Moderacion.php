...
use app\interfaces\ModeracionInterface;

class Moderacion implements ModeracionInterface {
	...
	*/
	...
	public function getPostsD() {
		...
		foreach ($data['datos'] as $pid => $post) {
		   ...
		   $data['datos'][$pid]["post_url"] = $tsCore->createLink('post', $post['post_id'], '#pp_' . $post['cid']);
		}
