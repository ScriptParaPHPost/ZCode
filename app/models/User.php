...
class User implements UserInterface {
	...
	public function __construct() {
		...
		if($this->autenticar->setSession()) $this->loadUser();
	}
	...
	*/
	public function validateTwoFactor() {
	   ...
	   if ((new TotpService)->verifyCodeSecret($data["user_secret_2fa"], $_POST['code']) || in_array($_POST['code'], $recovery)) {
	      // Si la verificación es exitosa, actualizar la sesión
	      $this->autenticar->sessionUpdate($data['user_id'], $rem, true);
