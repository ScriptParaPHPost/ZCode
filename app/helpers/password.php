...
/*++++++++ = ++++++++*/
	...
	// hash
	$hash = db_exec([__FILE__, __LINE__], 'query', 'SELECT * FROM @contacts WHERE hash = \''.$tsCore->setSecure($key).'\' AND user_email = \''.$email.'\' AND type = '.(int)$type.' ORDER BY id DESC LIMIT 1') or exit( show_error('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'db') );
	...
	if($type == 2){
			if(db_exec([__FILE__, __LINE__], 'query', 'UPDATE @miembros SET user_activo = \'1\' WHERE user_id = \''.$data['user_id'].'\'') or exit( show_error('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'db') )) {
			db_exec([__FILE__, __LINE__], 'query', 'DELETE FROM @contacts WHERE user_id = \''.$data['user_id'].'\'') or exit( show_error('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'db') );
		...
		if($_POST){
			...
			db_exec([__FILE__, __LINE__], 'query', 'UPDATE @miembros SET user_password = \''.md5(md5($_POST['pass']).strtolower($data['user_name'])).'\' WHERE user_id = \''.$data['user_id'].'\'') or exit( show_error('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'db') );
			db_exec([__FILE__, __LINE__], 'query', 'DELETE FROM @contacts WHERE user_id = \''.$data['user_id'].'\'') or exit( show_error('Error al ejecutar la consulta de la l&iacute;nea '.__LINE__.' de '.__FILE__.'.', 'db') );
