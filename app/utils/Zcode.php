...
class Zcode {
   ...
   */
	...
	*/
	 ...
	 * @return string
	*/
	public function createPassword(string $user = '', string $pass = '', ?string $hashed = ''): string|bool {
		$options = ['cost' => 12];
		$password = htmlspecialchars(trim($user) . trim($pass));
		// Contraseña hasheada
		return empty($hashed) ? password_hash($password, PASSWORD_DEFAULT, $options) : password_verify($password, $hashed);
	}
